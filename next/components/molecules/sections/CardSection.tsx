  ...
  return (
    <Section anchor={anchor} title={title}>
      {isLoading ? (
        <div
          className={cx(
            'grid px-xStandard grid-cols-1 gap-x-6 lg:gap-x-10 gap-y-14 md:grid-cols-2 lg:grid-cols-3',
            {
              'py-yHigh': !title,
              'pb-yHigh': title,
            }
        ...
        <>
          ...
          {loadmoreButton && (
            <div className="flex justify-center py-12">{loadmoreButton}</div>
          )}
        </>
      ) : (
        <div className="relative px-xStandard pt-yStandard pb-[calc(var(--padding-y)+40px)]">
          <p className="text-md">
            {noItemsMessage ?? t('common.nothingToShow')}
          </p>
        </div>
      )}
    </Section>
