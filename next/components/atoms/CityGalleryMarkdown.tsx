  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h4: ({ children }) => <h4 className="pb-8 font-semibold text-md">{children}</h4>,
        h5: ({ children }) => <h5 className="pb-6 text-sm font-semibold">{children}</h5>,
        h6: ({ children }) => <h6 className="pb-6 text-sm font-semibold text-gmbGray">{children}</h6>,
        p: ({ children }) => <p className="pb-6 text-md">{children}</p>,
        ul: ({ children }) => <ul className="pb-6 list-disc pl-14 text-md">{children}</ul>,
        ol: ({ children }) => <ol className="pb-6 list-decimal pl-14 text-md">{children}</ol>,
        strong: ({ children }) => (
          <strong className="font-medium" style={{ color: accentColor }}>
            {children}
          </strong>
        ),
        blockquote: ({ children }) => <div className="pb-6 pl-10 border-l-4">{children}</div>,
        img: ({ src, alt }) => <div className="relative">{src && <ImageFigure src={src} alt={alt} />}</div>,
        a: ({ children, href }) => {
