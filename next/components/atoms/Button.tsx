...
const Button = ({
  ...
  const styles = {
    className: cx(className, 'flex justify-center border-2 border-solid text-center text-btn font-medium uppercase', {
      ...
      'group-hover:bg-gmbDark group-hover:text-white': groupHover && color === 'dark' && size !== 'link',
      'border-white text-white hover:bg-white hover:text-gmbDark focus:bg-white focus:text-gmbDark':
        color === 'light' && size !== 'link',
      'group-hover:bg-white group-hover:text-gmbDark': groupHover && color === 'light' && size !== 'link',
