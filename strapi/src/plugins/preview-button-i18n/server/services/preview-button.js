...
module.exports = ({ strapi }) => ({
  ...
  getPreviewUrls(entity, contentType) {
    ...
    const localizedPublishedUrls = new Object();
    Object.keys(publishedBasePath).forEach((locale) => {
      localizedPublishedUrls[locale] = buildUrl(
        process.env.STRAPI_PREVIEW_PUBLISHED_URL,
        publishedBasePath[locale],
        targetFieldValue,
        publishedQuery
      );
    });
